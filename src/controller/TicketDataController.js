...
export default class TicketDataController {
    ...
    async saveTicket(data){
        ...
        return new Promise(async (resolve) => {
            var sql = `select * from ticket where ticket_code='`+data.ticketDetail.ticket_code+`' and Date(created_at)=Date('`+this.getDate()+`'`;
            await window.api.getData(sql).then(results=>{
                if(results.length > 0){
                    var savedData = results[0];
                    var ticketid = savedData.sync_id;
                    ...
                    window.api.getSyncUniqueId().then(csyn => {
                        ...
                        var ticket_input = {
                            ...
                            isDelete: 0,
                            tips_totalamt : data.tipsAmount,
                            tips_type: data.tipsType,
                            tips_percent: data.tipsPercent,
                            discount_id: data.ticketDiscount.discount_id,
                            discount_type: data.ticketDiscount.discount_type,
                            discount_value: data.ticketDiscount.discount_value,
                            discount_totalamt : data.ticketDiscount.discount_amt,
                            notes: data.ticketDetail.notes
                        }

                        thisobj.saveData({table_name:'ticket', data:ticket_input}).then(r=>{
                            data.ticketref_id = ticket_input.sync_id;
                            thisobj.saveTicketServices(data, resolve);
                        });
                    });
                }
            });
    ...
    saveTicketDetail(input, resolve, savedData){
        ...
        var ticket_input = {
            ...
            discounts: input.discountAmount,
            paid_status : '',
            created_at: this.getDate(),
            updated_at: this.getDate(),
            created_by : this.getUserId(),
            updated_by: this.getUserId(),
            businessId: this.getBusinessId(),
            total_tax: input.taxAmount,
            grand_total : input.grandTotal, 
            isDelete: 0,
            tips_totalamt : input.tipsAmount,
            tips_type: input.tipsType,
            tips_percent: input.tipsPercent,
            discount_id: input.ticketDiscount.discount_id,
            discount_type: input.ticketDiscount.discount_type,
            discount_value: input.ticketDiscount.discount_value,
            discount_totalamt : input.ticketDiscount.discount_amt,
        ...
        this.updateData({table_name:'ticket', data: ticket_input, query_field:'sync_id', query_value:savedData.sync_id}).then(r=>{
            ...
            console.log(obj)
            thisobj.saveTicketServices(obj, resolve);
        });
    }

    async saveTicketServices(input, resolve){
    ...
    saveEmptyTicket(data){
        var thisobj = this;
        return new Promise(async (resolve) => {
            window.api.getSyncUniqueId().then(csyn => {
                var syncid = data.ticketDetail.sync_id !== undefined && data.ticketDetail.sync_id !== null && data.ticketDetail.sync_id !=='' ? data.ticketDetail.sync_id:csyn.syncid;
                var ticket_input = {
                    ticket_code: data.ticketDetail.ticket_code,
                    sync_id: syncid,
                    sync_status:0,
                ...
                if(data.ticketDetail.sync_id !== undefined && data.ticketDetail.sync_id !== null && data.ticketDetail.sync_id !== ''){
                    syncid = data.ticketDetail.sync_id;
                    
                    thisobj.updateTicketData({table_name:'ticket', data:ticket_input, query_field:'sync_id', query_value:syncid}).then(r=>{ 
                        window.api.getTicketCode().then(res=>{ 
                            if(res.ticketid !== ''){
                                var ticket_code = String(res.ticketid).padStart(4, '0'); 
                                window.api.invoke('log', "Crate ticket from transfer single service :: "+ticket_code);
                                window.api.getSyncUniqueId().then(csyn => {
                                    var syncid = csyn.syncid;
                                    var ticket_input = {
                                    ...
                                    thisobj.saveData({table_name:'ticket', data:ticket_input}).then(r=>{ 
                                        ticket_input["id"] = ticket_input["sync_id"];
                                        resolve(ticket_input)
                                    });
                                });
                            } 
                        })
                    ...
                    thisobj.saveData({table_name:'ticket', data:ticket_input}).then(r=>{ 
                        window.api.getTicketCode().then(res=>{ 
                            if(res.ticketid !== ''){
                                var ticket_code = String(res.ticketid).padStart(4, '0'); 
                                window.api.invoke('log', "ELSE Crate ticket from transfer single service :: "+ticket_code);
                                window.api.getSyncUniqueId().then(csyn => {
                                    var syncid = csyn.syncid;
                                    var ticket_input = {
                                        ticket_code: ticket_code,
    ...
    saveTransferUpdateTicket(input){
        var thisobj = this;
        return new Promise(async (resolve) => { 
            var ticket_input = { 
                sync_status:0,
                technician_id: input.ticketowner.id,
                customer_id: input.customer_detail !== undefined ? input.customer_detail.id : '', 
                subtotal: input.subTotal,
                discounts: input.discountAmount,
                paid_status : '', 
                updated_at: this.getDate(), 
            ...
            this.updateTicketData({table_name:'ticket', data: ticket_input, query_field:'sync_id', query_value:input.ticketDetail.sync_id}).then(r=>{
                console.log("AASDASDASDASDADADASDASDASDASDASDAD")
                var obj = Object.assign({}, input);
                obj.ticketref_id = input.ticketDetail.sync_id;
                console.log(obj)
                thisobj.saveTicketServices(obj, resolve); 
            });
        });
    ...
    updateTransferedService(input){
        console.log(input)
        return new Promise(async (resolve) => { 
            this.updateData({table_name:'ticket_services', data:{isActive:2}, query_field:'sync_id', query_value:input.id}).then(r=>{
                console.log("TICKET SERVICE UPDATE");
                this.updateData({table_name:'employee_commission_detail', data:{isActive:2}, query_field:'ticketserviceref_id', query_value:input.id}).then(r=>{
                    console.log("TICKET SERVICE COMMISSION UPDATE");
                    resolve('success')
                })
            })
        });
