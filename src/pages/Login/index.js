...
onLoginLocal() {
    ...
    this.state.dataManager.getData("select * from users where passcode='"+this.state.passcode+"'").then(res=>{
        if(res.length>0) {
            let data = res[0]
            let userdata = {}
            ...
            let time = new Date()
            let sql = "update staff_clockLog set isActive=0 where staff_id="+userdata.id
            console.log("pdate staff_clockLog:::",sql)
            this.state.dataManager.saveData(sql).then(r=>{ 
        ...
        <div>
            ...
            <div>
                <div className='container synccontainer'>
                    ...
                    <div className='loginScreen_numberpad'> 
                        <Card style={{borderRadius: 16, boxShadow:'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px',objectFit: 'cover'}}>
