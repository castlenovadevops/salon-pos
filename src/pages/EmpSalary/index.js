    ...
    handlePageEvent(pagename){
      ...
      viewReport(row){
        ...
        this.setState({ selectedEmp: row }, ()=>{
            ...
            var sql = `select ts.id,strftime('%m/%d/%Y', (select created_at from ticket where sync_id=ts.ticketref_id)) AS ticket_date, (select sum(totalamount) from employee_commission_detail where ticketserviceref_id = ts.sync_id and (cash_type_for='service' or cash_type_for='product')) as Amount, ts.tips_amount as Tips, ts.total_discount_amount as Discount from ticket_services as ts where ts.ticketref_id in (select  ticketref_id from ticket_payment where DATE(paid_at) between '`+ this.state.from_date.toISOString().substring(0,10)+`' and '`+ this.state.to_date.toISOString().substring(0,10)+`' and ticketref_id in (select sync_id from ticket where isDelete=0 and paid_status='paid') ) and ts.isActive=1 and ts.employee_id = `+empid;
            ...
            if(this.state.from_date === this.state.to_date){
                sql = `select ts.id,strftime('%m/%d/%Y', (select created_at from ticket where sync_id=ts.ticketref_id)) AS ticket_date, (select sum(totalamount) from employee_commission_detail where ticketserviceref_id = ts.sync_id and (cash_type_for='service' or cash_type_for='product')) as Amount, ts.tips_amount as Tips, ts.total_discount_amount as Discount from ticket_services as ts where ts.ticketref_id in (select  ticketref_id from ticket_payment where DATE(paid_at) = '`+ this.state.from_date.toISOString().substring(0,10)+`' and ticketref_id in (select sync_id from ticket where isDelete=0 and paid_status='paid') ) and ts.isActive=1 and ts.employee_id = `+empid
